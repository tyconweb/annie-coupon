...
if( !function_exists( 'meta_title' ) ) {
    ...
    function meta_title() {
        if($_SERVER['REQUEST_URI'] == '/'){
            return \query\main::get_option( 'sitetitle' );
        }elseif($_SERVER['REQUEST_URI'] == '/coupons'){
            $desc = \query\main::get_option( 'meta_coupon_title' );
            return $desc;
        } elseif($_SERVER['REQUEST_URI'] == '/stores'){
            $desc = \query\main::get_option( 'meta_store_title' );
            return $desc;   
        } elseif($_SERVER['REQUEST_URI'] == '/categories'){
            $desc = \query\main::get_option( 'meta_category_title' );
            return $desc;   
        }elseif (mb_strpos($_SERVER['REQUEST_URI'],'store') !== false){
            $data = storeData(explode('/',$_GET['lcp'])[1]);
            return !empty($data['meta_title']) ? $data['meta_title'] : \query\main::get_option( 'meta_store_title' );
        } elseif(isset($_GET['couponId']) && !empty($_GET['couponId'])){
            $data = couponData($_GET['couponId']);
            return !empty($data['meta_title']) ? $data['meta_title'] : \query\main::get_option( 'meta_coupon_title' );    
        } elseif(isset($_GET['category']) && !empty($_GET['category'])){
            $data = categoryData($_GET['category']);
            return !empty($data['meta_title']) ? $data['meta_title'] : \query\main::get_option( 'meta_category_title' );    
        }
...
if( !function_exists( 'meta_keywords' ) ) {
    ...
    function meta_keywords() {
        if($_SERVER['REQUEST_URI'] == '/'){
            ...
            $data = storeData(explode('/',$_GET['lcp'])[1]);
            return !empty($data['meta_keywords']) ? $data['meta_keywords'] : \query\main::get_option( 'meta_store_keywords' );
        } elseif(isset($_GET['couponId']) && !empty($_GET['couponId'])){
            $data = couponData($_GET['couponId']);
            return !empty($data['meta_keywords']) ? $data['meta_keywords'] : \query\main::get_option( 'meta_coupon_keywords' );    
        } elseif(isset($_GET['category']) && !empty($_GET['category'])){
            $data = categoryData($_GET['category']);
            return !empty($data['meta_keywords']) ? $data['meta_keywords'] : \query\main::get_option( 'meta_category_keywords' );    
        }
        return meta_default( '', \query\main::get_option( 'meta_keywords' ) );
...
if( !function_exists( 'meta_description' ) ) {
    ...
    function meta_description() {
        if($_SERVER['REQUEST_URI'] == '/'){
            ...
            $data = storeData(explode('/',$_GET['lcp'])[1]);
            return !empty($data['meta_desc']) ? $data['meta_desc'] : \query\main::get_option( 'meta_store_desc' );
        } elseif(isset($_GET['couponId']) && !empty($_GET['couponId'])){
            $data = couponData($_GET['couponId']);
            return !empty($data['meta_desc']) ? $data['meta_desc'] : \query\main::get_option( 'meta_coupon_desc' );    
        } elseif(isset($_GET['category']) && !empty($_GET['category'])){
            $data = categoryData($_GET['category']);
            return !empty($data['meta_desc']) ? $data['meta_desc'] : \query\main::get_option( 'meta_category_desc' );    
        }
        return meta_default( '', \query\main::get_option( 'meta_description' ) );
