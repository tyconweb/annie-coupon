...
public static function blogs( $blogs = array() ) {
    return self::have_blogs( $blogs, array( 'only_count' => true ) );
}
...
public static function sliders( $categories = array() ) {
    return self::have_sliders( $categories, array( 'only_count' => true ) );
}
...
public static function slider_avatar( $text ) {
...
public static function category_avatar( $text ) {
...
public static function category_info( $id = 0, $special = array() ) {
    ...
    $stmt = $db->stmt_init();
    $stmt->prepare( "SELECT id, subcategory, user, (SELECT name FROM " . DB_TABLE_PREFIX . "users WHERE id = c.user), name, description,icon, (SELECT COUNT(*) FROM " . DB_TABLE_PREFIX . "stores WHERE category = c.id), url_title, meta_title, meta_keywords, meta_desc, extra, date FROM " . DB_TABLE_PREFIX . "categories c WHERE id = ? OR url_title = ?" );
    $stmt->bind_param( "is", $id, $id );
    $stmt->execute();
    $stmt->bind_result( $id, $subcategory, $user, $user_name, $name, $description, $icon, $stores, $url_title, $meta_title, $meta_keywords, $meta_desc, $extra, $date );
    $stmt->fetch();
    $stmt->close();
...
public static function slider_exists( $slider_id = 0 ) {

    global $db, $GET;
    $slider_id = empty( $slider_id ) ? $GET['id'] : $slider_id;
    


    $stmt = $db->stmt_init();
    $stmt->prepare( "SELECT COUNT(*) FROM " . DB_TABLE_PREFIX . "sliders WHERE id = ?");
    ...
    if( $count > 0 ) {
    // echo "<pre> ddd";print_r($count);die;
        return true;
    }
...
/* GET INFORMATION ABOUT A SLIDER */

public static function slider_info( $slider_id = 0 ) {
    ...
    $stmt = $db->stmt_init();
    $stmt->prepare( "SELECT id, user, (SELECT name FROM " . DB_TABLE_PREFIX . "users WHERE id = r.user), title, image, lastupdate_by, (SELECT name FROM " . DB_TABLE_PREFIX . "users WHERE id = r.lastupdate_by), lastupdate, visible, date FROM " . DB_TABLE_PREFIX . "sliders r WHERE id = ?" );
    $stmt->bind_param( "i", $slider_id );
    $stmt->execute();
    $stmt->bind_result( $id, $user, $user_name, $title, $image, $lastupdate_by, $lastupdate_by_name, $last_update, $visible, $date );
    $stmt->fetch();
    $stmt->close();

    return (object) array( 'ID' => $id, 'user' => $user, 'user_name' => esc_html( $user_name ), 'title' => esc_html( $title ) , 'image' => esc_html( $image ), 'lastupdate_by' => $lastupdate_by, 'lastupdate_by_name' => esc_html( $lastupdate_by_name ), 'last_update' => $last_update, 'visible' => (boolean) $visible, 'date' => $date );
...
public static function group_stores( $category = array(), $special = array() ) {

    $array = array();
    foreach( \query\main::while_stores() as $c ) {
        $array['cat_' . $c->ID]['info'] = $c;
    }
...
public static function have_sliders( $category = array(), $special = array() ) {
    ...
    $stmt = $db->stmt_init();
    $stmt->prepare( "SELECT COUNT(*) FROM " . DB_TABLE_PREFIX . "sliders" . ( empty( $where ) ? '' : ' WHERE ' . implode( ' AND ', array_filter( $where ) ) ) );
    $stmt->execute();
    $stmt->bind_result( $count );
    $stmt->fetch();
    $stmt->close();
...
public static function while_sliders( $category = array() ) {
    ...
    $stmt = $db->stmt_init();
    $stmt->prepare( "SELECT id, user, title, image, visible, date FROM " . DB_TABLE_PREFIX . "sliders" . ( empty( $where ) ? '' : ' WHERE ' . implode( ' AND ', array_filter( $where ) ) ) . ( empty( $orderby ) ? '' : ' ORDER BY ' . implode( ', ', array_filter( $orderby ) ) ) . ( empty( $limit ) ? '' : ' LIMIT ' . implode( ',', $limit ) ) );
    $stmt->execute();
    $stmt->bind_result( $id, $user, $title, $image, $visible, $date );

    $data = array();
    while( $stmt->fetch() ) {
        $data[] = (object) array( 'ID' => $id, 'user' => $user, 'title' => esc_html( $title ), 'image' => esc_html( $image ),  'visible' => (boolean) $visible, 'date' => $date );
    }
...
// BLOG MODULE FUNCTIONS
public static function blog_avatar( $text ) {
...
public static function blog_exists( $id = 0 ) {
    ...
    $stmt = $db->stmt_init();
    $stmt->prepare( "SELECT COUNT(*) FROM " . DB_TABLE_PREFIX . "blogs WHERE id = ?" );
    $stmt->bind_param( "i", $id);
    $stmt->execute();
    $stmt->bind_result( $count );
    $stmt->fetch();
...
/* GET INFORMATION ABOUT BLOG */

public static function blog_info( $id = 0, $special = array() ) {
    ...
    $stmt = $db->stmt_init();
    $stmt->prepare( "SELECT id, title, short_description, content, section_name ,display_name , meta_title, meta_keywords, meta_desc, image, created FROM " . DB_TABLE_PREFIX . "blogs c WHERE id = ?" );
    $stmt->bind_param( "i", $id);
    $stmt->execute();
    $stmt->bind_result( $id, $title, $short_description, $content, $section_name , $display_name , $meta_title, $meta_keywords, $meta_desc , $image, $date);
    $stmt->fetch();
    $stmt->close();

    // special
    $useem = ( isset( $special['no_emoticons'] ) && $special['no_emoticons'] ) ? false : (boolean) \query\main::get_option( 'smilies_blogs' );
    $usesh = ( isset( $special['no_shortcodes'] ) && $special['no_shortcodes'] ) ? false : true;
    $useec = ( isset( $special['no_escape'] ) && $special['no_escape'] ) ? false : true;
    $usefl = ( isset( $special['no_filters'] ) && $special['no_filters'] ) ? false : true;

    return (object) value_with_filter( 'blog_info_values', array( 
            'ID' => $id, 
            'title' => \site\content::title( 'blog_title_single', $title, $useem, $useec, $usefl ), 
            'section_name' => \site\content::title( 'blog_section_name_single', $section_name, $useem, $useec, $usefl ), 
            'display_name' => \site\content::title( 'blog_display_name_single', $display_name, $useem, $useec, $usefl ), 
            'short_description' => \site\content::content( 'blog_short_description_single', $short_description, $useem, $useec, $usefl ), 
            'content' => \site\content::content( 'blog_content_single', $content, $useem, $usesh, false, $useec, $usefl ),
            'image'=> $image,
            'meta_title' => esc_html( $meta_title ), 
            'meta_keywords' => esc_html( $meta_keywords ), 
            'meta_description' => esc_html( $meta_desc ), 
            'date' => $date, 
            'link' => ( defined( 'SEO_LINKS' ) && SEO_LINKS ? \site\utils::make_seo_link( 'blog', $title, 'blog', $id, '.php' ) : $GLOBALS['siteURL'] . '?blog=' . $id ) ) );

}

/* NUMBER OF BLOGS */

public static function have_blogs( $blog = array(), $special = array() ) {

    global $db;

    $blogs = \site\utils::validate_user_data( $blog );

    $where = array();

    /* WHERE / ORDER BY */

    if( !empty( $blogs['ids'] ) && strcasecmp( $blogs['ids'], 'all' ) != 0 ) {
        $arr = array_filter( array_map( function( $w ){
            return (int) $w;
        }, explode( ',', $blogs['ids'] ) ));
        if( !empty( $arr ) )
        $where[] = 'id IN(' . \site\utils::dbp( implode(',', $arr) ) . ')';
        if( !isset( $blogs['orderby'] ) ) {
            $orderby[] = 'field(id,' . \site\utils::dbp( implode(',', $arr) ) . ')';
        }
    }
    if( !empty( $blogs['search'] ) ) {
        $search = implode( '.*', explode( ' ', trim( $blogs['search'] ) ) );
        $where[] = 'CONCAT(title,short_description) REGEXP "' . \site\utils::dbp( $search ) . '"';
    }

    /* */

    $stmt = $db->stmt_init();
    $stmt->prepare( "SELECT COUNT(*) FROM " . DB_TABLE_PREFIX . "blogs" . ( empty( $where ) ? '' : ' WHERE ' . implode( ' AND ', array_filter( $where ) ) ) );
    $stmt->execute();
    $stmt->bind_result( $count );
    $stmt->fetch();
    $stmt->close();
    ...
    $pags = array();
    $pags['results'] = $count;
    $pags['per_page'] = ( !empty( $blogs['per_page'] ) ? (int) $blogs['per_page'] : \query\main::get_option( 'items_per_page' ) );
    $pags['pages'] = ceil( $pags['results'] / $pags['per_page'] );
    $page = ( !empty( $blogs['page'] ) ? (int) $blogs['page'] : ( !empty( $_GET['page'] ) ? (int) $_GET['page'] : 1 ) );
    if( $page < 1 ) $page = 1;
    if( $page > $pags['pages'] ) $page = $pags['pages'];
    $pags['page'] =    $page;
    if( $pags['pages'] > $pags['page'] ) $pags['next_page'] = \site\utils::update_uri( '', array( 'page' => ($pags['page']+1) ) );
...
public static function while_blogs( $blog = array(), $special = array() ) {

    global $db;

    /** make or not seo links */
    $seo_link = defined( 'SEO_LINKS' ) && SEO_LINKS ? true : false;
    list( $seo_link_blog, $extension ) = array( \query\main::get_option( 'seo_link_blog' ), \query\main::get_option( 'extension' ) );
    ...
    if( isset( $blogs['max'] ) ) {
        ...
        $page = ( !empty( $blogs['page'] ) ? (int) $blogs['page'] : ( !empty( $_GET['page'] ) ? (int) $_GET['page'] : 1 ) );
        $per_page = ( isset( $blogs['per_page'] ) ? (int) $blogs['per_page'] : \query\main::get_option( 'items_per_page' ) );
        $offset = isset( $page ) && $page > 1 ? ( $page - 1 ) * $per_page : 0;
    ...
    if( isset( $blogs['orderby'] ) ) {
        $order = array_map( 'trim', explode( ',', strtolower( $blogs['orderby'] ) ) );
        $custom_orderby_clause = value_with_filter( 'blogs_orderby_clause', array(
            'rand'          => 'RAND()',
            'title'          => 'b.title',
            'title desc'     => 'b.title DESC',
            'created'          => 'b.created',
            'created desc'     => 'b.created DESC'
        ) );
    ...
    $stmt = $db->stmt_init();
    $stmt->prepare( "SELECT id, title, short_description, content, section_name ,display_name , meta_title, meta_keywords, meta_desc, image, created, publish FROM " . DB_TABLE_PREFIX . "blogs b" . ( empty( $where ) ? '' : ' WHERE ' . implode( ' AND ', array_filter( $where ) ) ) . ( empty( $orderby ) ? '' : ' ORDER BY ' . implode( ', ', array_filter( $orderby ) ) ) . ( empty( $limit ) ? '' : ' LIMIT ' . implode( ',', $limit ) ) );
    $stmt->execute();
    $stmt->bind_result(  $id, $title, $short_description, $content, $section_name , $display_name , $meta_title, $meta_keywords, $meta_desc , $image, $date,$publish);

    $data = array();
    while( $stmt->fetch() ) {

        $data[] = (object) value_with_filter( 'blog_info_values', array( 
            'ID' => $id, 
            'title' => \site\content::title( 'blog_title_single', $title, $useem, $useec, $usefl ), 
            'section_name' => \site\content::title( 'blog_section_name_single', $section_name, $useem, $useec, $usefl ), 
            'display_name' => \site\content::title( 'blog_display_name_single', $display_name, $useem, $useec, $usefl ), 
            ...
            'meta_description' => esc_html( $meta_desc ), 
            'publish' => $publish, 
            'link' => ( defined( 'SEO_LINKS' ) && SEO_LINKS ? \site\utils::make_seo_link( 'blog', $title, $seo_link, $id, '.php') : $GLOBALS['siteURL'] . '?cat=' . $id ) ) );
...
public static function group_blogs( $blog = array(), $special = array() ) {

    $array = array();

    foreach( \query\main::while_blogs( $blog, $special ) as $c ) {
        $array['blog_' . $c->ID]['info'] = $c;
    }
