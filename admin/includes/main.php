...
public function default_links() {
    ...
    if( ( $ab = ab_to( array( 'sliders' => array( 'view', 'add' ) ) ) ) && list( $sliders_view, $sliders_add ) = $ab ) {
        $nav['sliders']['name'] = '<a href="?route=sliders.php">' . t( 'sliders', "Sliders" ) . '</a>';
        $nav['sliders']['class'] = 'plugin1';
        $nav['sliders']['position'] = 8;
        if( $sliders_add ) {
            $nav['sliders']['subnav']['add'] = '<a href="?route=sliders.php&amp;action=add">' . t( 'sliders_add', "Add Slider" ) . '</a>';
        }
        if( $sliders_view ) {
            $nav['sliders']['subnav']['list'] = '<a href="?route=sliders.php&amp;action=list">' . t( 'sliders_view', "View Sliders" ) . '</a>';
        }
    }
    if( ( $ab = ab_to( array( 'blogs' => array( 'view', 'add' ) ) ) ) && list( $blogs_view, $blogs_add ) = $ab ) {
        $nav['blogs']['name'] = '<a href="?route=blogs.php">' . t( 'blogs', "Blogs" ) . '</a>';
        $nav['blogs']['class'] = 'plugin1';
        $nav['blogs']['position'] = 8;
        if( $blogs_add ) {
            $nav['blogs']['subnav']['add'] = '<a href="?route=blogs.php&amp;action=add">' . t( 'blogs_add', "Add Blog" ) . '</a>';
        }
        if( $blogs_view ) {
            $nav['blogs']['subnav']['list'] = '<a href="?route=blogs.php&amp;action=list">' . t( 'blogs_view', "View Blogs" ) . '</a>';
        }
    }
...
public function blog_list_markup( $item, $links ) {

    $lists = get( 'admin-list-style' );

    $links = value_with_filter( 'admin_list_blog', $links, $item );
// echo "<pre>";print_r($item);die;
    if( isset( $lists['blog'] ) && function_exists( $lists['blog'] ) ) {
        return $lists['blog']($item['item'], $links);
    }
    ...
    <img src="' . \query\main::blog_avatar( $item->image ) . '" alt="" />
    <div class="info-div">

    <h2>' .  mb_strimwidth($item->title, 0, 20, "...")  . '</h2>
    <h2>' . ( $item->publish ? '<span class="msg-success">' . t( 'published', "Published" ) . '</span>' : '<span class="msg-error">' . t( 'notpublished', "Not Published" ) . '</span>' ) . '</h2>
...
public function slider_list_markup( $item, $links ) {

    $lists = get( 'admin-list-style' );

    $links = value_with_filter( 'admin_list_reward', $links, $item );

    if( isset( $lists['slider'] ) && function_exists( $lists['slider'] ) ) {
        return $lists['slider']($item['item'], $links);
    }
...
public function blog_fields( $info = object, $csrf = '' ) {

    do_action( 'before_blog_fields', $info );

    global $add_blog_fields;

    $custom_fields  = array();
    $sections       = array();

    if( !empty( $add_blog_fields ) && is_array( $add_blog_fields) ) {
        foreach( $add_blog_fields as $id => $link ) {
            $custom_fields[$id]['position'] = isset( $link['position'] ) ? $link['position'] : 99;
            $extra = widgets::build_extra( $link['fields'], ( isset( $info->extra ) && is_array( $info->extra ) ? $info->extra : array() ), '', 'extra', $info );
            $custom_fields[$id]['markup'] = $extra['markup'];
            $sections = array_merge( $sections, $extra['sections'] );        }
    ...
    return array( 'fields' => self::default_blog_fields( $info, $csrf ) + $custom_fields, 'sections' => $sections );
}

public function default_blog_fields( $info = object, $csrf = '' ) {
    ...
    $blog = '';
    // Name
    $fields['title']['position'] = 1;
    $fields['title']['markup'] = '<div class="row"><span>' . t( 'form_name', "Name" ) . ':</span><div><input type="text" name="title" value="' . ( isset( $info->title ) ? $info->title : '' ) . '" required /></div></div>';

    // Short Desc
    $fields['short_description']['position'] = 2;
    $fields['short_description']['markup'] = '<div class="row"><span>' . t( 'form_short_description', "Short Description" ) . ':</span><div><textarea name="short_description" style="min-height:100px;">' . ( isset( $info->short_description ) ? $info->short_description : '' ) . '</textarea></div></div>';

    // Section Name
    $fields['section_name']['position'] = 3;
    $fields['section_name']['markup'] = '
    <div class="row">
        <span>' . t( 'form_section_name', "Section Name" ) . ':</span>
        <div cc="'.(isset($info->section_name) && $info->section_name == 'VACATION & LEISURE TIME' ? 'selected' : '').'">
            <select name="section_name" class="" aria-label="Default select example">
              <option>Select </option>
              <option ' . ( isset( $info->section_name ) && $info->section_name ==  'LIFESTYLE' ? 'selected' : '' ) . ' value="LIFESTYLE">LIFESTYLE</option>
              <option ' . ( isset( $info->section_name ) && $info->section_name ==  'SHOPPING HACKS' ? 'selected' : '' ) . ' value="SHOPPING HACKS">SHOPPING HACKS</option>
              <option ' . ( isset( $info->section_name ) && 
                    strpos($info->section_name, 'VACATION') !== false ? 'selected' : '' ) . ' value="VACATION & LEISURE TIME">VACATION & LEISURE TIME</option>
              <option ' . ( isset( $info->section_name ) && $info->section_name ==  'AT HOME' ? 'selected' : '' ) . ' value="AT HOME">AT HOME</option>
            </select>
        </div>
    </div>';

    // Display Name
    $fields['display_name']['position'] = 4;
    $fields['display_name']['markup'] = '<div class="row"><span>' . t( 'form_display_name', "Display Name" ) . ':</span><div><input type="text" name="display_name" value="' . ( isset( $info->display_name ) ? $info->display_name : '' ) . '" required /></div></div>';

    // Content
    $fields['content']['position'] = 5;
    $fields['content']['markup'] = '<div class="row"><span>' . t( 'form_description', "Content" ) . ':</span><div><textarea name="content" style="min-height:100px;">' . ( isset( $info->content ) ? $info->content : '' ) . '</textarea></div></div>';

    return value_with_filter( 'default_blog_fields', $fields );
