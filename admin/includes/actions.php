...
public static function add_category( $opt = array() ) {
    ...
    $stmt = $db->stmt_init();
    $stmt->prepare( "INSERT INTO " . DB_TABLE_PREFIX . "categories (subcategory, user, name, description, meta_title, meta_keywords, meta_desc, icon,extra, date) VALUES (?, ?, ?, ?, ?, ?, ?,?, ?, NOW())" );

    $image = \site\images::upload( @$_FILES['icon'], 'icon_', array( 'path' => DIR . '/', 'current' => '' ) );

    $extra = \site\utils::array_sanitize( $opt['extra'] );
    $extra = @serialize( $extra );

    $stmt->bind_param( "iisssssss", $opt['category'], $GLOBALS['me']->ID, $opt['name'], $opt['description'], $opt['meta_title'], $opt['meta_keywords'], $opt['meta_desc'],$image, $extra );
...
public static function edit_category( $id, $opt = array() ) {
    ...
    $category = \query\main::category_info( $id );
    $icon = \site\images::upload( @$_FILES['icon'], 'icon_', array( 'path' => DIR . '/', 'current' => $category->icon ) );

    $stmt = $db->stmt_init();
    $stmt->prepare( "UPDATE " . DB_TABLE_PREFIX . "categories SET subcategory = ?, name = ?, description = ?, icon = ? , meta_title = ?, meta_keywords = ?, meta_desc = ?, extra = ? WHERE id = ?" );

    $extra = \site\utils::array_sanitize( $opt['extra'] );
    $extra = @serialize( $extra );

    $stmt->bind_param( "isssssssi", $opt['category'], $opt['name'], $opt['description'], $icon ,$opt['meta_title'], $opt['meta_keywords'], $opt['meta_desc'], $extra, $id );
    $execute = $stmt->execute();
    $stmt->close();
...
public static function delete_category( $id ) {
    ...
    foreach( $id as $ID ) {
        ...
        if( \query\main::category_exists( $ID ) ) {

        $slider = \query\main::category_info( $ID );

        $stmt->bind_param( "i", $ID );

            if( $stmt->execute() && !empty( $slider->icon ) && !preg_match( '/^http(s)?/i', $slider->icon ) ) {
                @unlink( DIR . '/' . $slider->icon );
            }
...
public static function add_slider( $opt = array() ) {
    ...
    $stmt = $db->stmt_init();
    $stmt->prepare( "INSERT INTO " . DB_TABLE_PREFIX . "sliders (user, title, image, lastupdate_by, lastupdate, visible, date) VALUES (?, ?, ?, ?,  NOW(), ?, NOW())" );

    $image = \site\images::upload( @$_FILES['logo'], 'slider_', array( 'path' => DIR . '/', 'current' => '' ) );


    $stmt->bind_param( "issii", $GLOBALS['me']->ID, $opt['name'], $image, $GLOBALS['me']->ID, $opt['publish'] );
    $execute = $stmt->execute();
    $stmt->close();
...
public static function edit_slider( $id, $opt = array() ) {
    ...
    $slider = \query\main::slider_info( $id );

    $avatar = \site\images::upload( @$_FILES['logo'], 'slider_', array( 'path' => DIR . '/', 'current' => $slider->image ) );

    $stmt = $db->stmt_init();
    $stmt->prepare( "UPDATE " . DB_TABLE_PREFIX . "sliders SET  title = ?, image = ?, lastupdate_by = ?, lastupdate = NOW(), visible = ? WHERE id = ?" );
    $stmt->bind_param( "ssiii",  $opt['name'],  $avatar,  $GLOBALS['me']->ID, $opt['publish'], $id );
    $execute = $stmt->execute();
    $stmt->close();
...
public static function delete_slider( $id ) {
    ...
    $stmt = $db->stmt_init();
    $stmt->prepare( "DELETE FROM " . DB_TABLE_PREFIX . "sliders WHERE id = ?" );

    foreach( $id as $ID ) {

        if( \query\main::slider_exists( $ID ) ) {

        $slider = \query\main::slider_info( $ID );

        $stmt->bind_param( "i", $ID );

            if( $stmt->execute() && !empty( $slider->image ) && !preg_match( '/^http(s)?/i', $slider->image ) ) {
                @unlink( DIR . '/' . $slider->image );
            }
...
/* SET ACTION TO SLIDER */

public static function action_slider( $action, $id ) {
    ...
    switch( $action ) {
        case 'publish':
            $stmt->prepare( "UPDATE " . DB_TABLE_PREFIX . "sliders SET visible = 1 WHERE id = ?" );
        break;

        case 'unpublish':
            $stmt->prepare( "UPDATE " . DB_TABLE_PREFIX . "sliders SET visible = 0 WHERE id = ?" );
        break;
...
public static function delete_slider_image( $id ) {
    ...
    $stmt = $db->stmt_init();
    $stmt->prepare( "UPDATE " . DB_TABLE_PREFIX . "sliders SET image = '' WHERE id = ?" );
...
public static function delete_category_image( $id ) {
    ...
    $stmt = $db->stmt_init();
    $stmt->prepare( "UPDATE " . DB_TABLE_PREFIX . "categories SET icon = '' WHERE id = ?" );
    ...
    if( $stmt->execute() && !empty( $category->icon ) && !preg_match( '/^http(s)?/i', $category->icon ) ) {
        @unlink( DIR . '/' . $category->icon );
    }
...
public static function add_blog( $opt = array() ) {

    global $db;

    if( !ab_to( array( 'blog' => 'add' ) ) ) return false;
    ...
    $stmt = $db->stmt_init();
    $stmt->prepare( "INSERT INTO " . DB_TABLE_PREFIX . "blogs (title, short_description, content, section_name ,display_name , meta_title, meta_keywords, meta_desc, image, created) VALUES (?, ?, ?, ?, ?, ?, ?,?, ?, NOW())" );

    $image = \site\images::upload( @$_FILES['image'], 'image_', array( 'path' => DIR . '/', 'current' => '' ) );
    // echo "<pre>";print_r($stmt);die;
    $stmt->bind_param( "sssssssss", $opt['title'],$opt['short_description'],$opt['content'],$opt['section_name'],$opt['display_name'], $opt['meta_title'], $opt['meta_keywords'], $opt['meta_desc'],$image);
    $execute = $stmt->execute();
    $insert_id = $stmt->insert_id;
    $stmt->close();
...
public static function edit_blog( $id, $opt = array() ) {

    global $db;

    if( !ab_to( array( 'blog' => 'edit' ) ) ) return false;
    ...
    $blog = \query\main::blog_info( $id );
    $image = \site\images::upload( @$_FILES['image'], 'image_', array( 'path' => DIR . '/', 'current' => $blog->image ) );

    $stmt = $db->stmt_init();
    $stmt->prepare( "UPDATE " . DB_TABLE_PREFIX . "blogs SET title = ?, short_description = ?, display_name = ?, section_name = ?, content = ?, image = ? , meta_title = ?, meta_keywords = ?, meta_desc = ? WHERE id = ?" );
    // print_r($stmt); exit;
    $stmt->bind_param( "sssssssssi",  $opt['title'], $opt['short_description'], $opt['display_name'], $opt['section_name'], $opt['content'], $image ,$opt['meta_title'], $opt['meta_keywords'], $opt['meta_desc'], $id );
    $execute = $stmt->execute();
    $stmt->close();
...
public static function edit_blog_url( $id, $opt = array() ) {
    ...
    $stmt->prepare( "SELECT COUNT(*) FROM " . DB_TABLE_PREFIX . "blogs WHERE id != ? AND url_title = ?" );
    $stmt->bind_param( "is", $id, $url );
    $stmt->execute();
    $stmt->bind_result( $count );
    $stmt->fetch();
    ...
    $stmt->prepare( "UPDATE " . DB_TABLE_PREFIX . "blogs SET url_title = ? WHERE id = ?" );
    $stmt->bind_param( "si", $url, $id );
    $execute = $stmt->execute();
    $stmt->close();
...
public static function delete_blog( $id ) {
    ...
    if( !ab_to( array( 'blog' => 'delete' ) ) ) return false;

    $id = (array) $id;

    $stmt = $db->stmt_init();
    $stmt->prepare( "DELETE FROM " . DB_TABLE_PREFIX . "blogs WHERE id = ?" );

    foreach( $id as $ID ) {

        if( \query\main::blog_exists( $ID ) ) {

        $slider = \query\main::blog_info( $ID );
        ...
        $stmt->bind_param( "i", $ID );
            ...
            do_action( 'admin_blog_deleted', $ID );
...
public static function delete_blog_image( $id ) {
    ...
    $stmt = $db->stmt_init();
    $stmt->prepare( "UPDATE " . DB_TABLE_PREFIX . "blogs SET image = '' WHERE id = ?" );
    ...
    if( $stmt->execute() && !empty( $blog->image ) && !preg_match( '/^http(s)?/i', $blog->image ) ) {
