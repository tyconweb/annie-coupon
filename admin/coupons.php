...
echo '
 <select name="store">
<option value="">' . t( 'all_stores', "All Stores" ) . '</option>';
foreach( \query\main::group_stores( array( 'max' => 0 ) ) as $cat ) {
    echo '<option value="' . $cat['info']->ID . '"' . ( isset( $_GET['store'] ) && $_GET['store'] == $cat['info']->ID ? ' selected' : '' ) . '>' . ts( $cat['info']->name ) . '</option>';
}
echo '</select>';
...
$p = \query\main::have_items( ( $options = value_with_filter( 'admin_view_items_args', array( 'per_page' => 999, 'store' => ( isset( $_GET['store'] ) ? $_GET['store'] : '' ), 'user' => ( isset( $_GET['user'] ) ? $_GET['user'] : '' ), 'categories' => ( isset( $_GET['category'] ) ? $_GET['category'] : '' ), 'show' => ( isset( $_GET['view'] ) ? $_GET['view'] : 'all' ), 'search' => ( isset( $_GET['search'] ) ? urldecode( $_GET['search'] ) : '' ) ) ) ) );
...
<ul class="elements-list" id="couponList">
