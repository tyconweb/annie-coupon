...
echo '        <div class="row"><span>' . t( 'form_image', "Icon" ) . ':</span> <div><input type="file" name="icon" /></div> </div>';
...
}else if( isset( $_GET['type'] ) && isset( $_GET['token'] ) && check_csrf( $_GET['token'], 'categories_csrf' ) ) {
// die;
if( $_GET['type'] == 'delete_image' ) {

    if( isset( $_GET['id'] ) )
    if( admin\actions::delete_category_image( $_GET['id'] ) ) {
    $info->image = '';
...
<form action="?route=categories.php&amp;action=edit&amp;id=' . $info->ID . '"  enctype="multipart/form-data" method="POST">';
...
<div>
<div style="display: table; margin-bottom: 2px;"><img src="' . \query\main::category_avatar( $info->icon ) . '" class="avt" alt="" style="display:table-cell;vertical-align:middle;max-width:120px;height:80px;margin: 0 20px 5px 0;" />
<div style="display: table-cell; vertical-align: middle; margin-left: 25px;">';
if( !empty( $info->icon ) ) echo '<a href="' . \site\utils::update_uri( '', array( 'type' => 'delete_image', 'token' => $csrf ) ) . '" class="btn" data-delete-msg="' . t( 'delete_msg', "Are you sure that you want to delete this?" ) . '">' . t( 'delete', "Delete" ) . '</a>';
echo '</div>
</div>
